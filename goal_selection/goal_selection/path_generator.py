...
def index_occupancy_grid(occupancy_grid: OccupancyGrid, index: OccupancyGridIndex):
    ...
    x_component = index.x + int(
        ROBOT_FORWARDS_BACKWARDS_POSITION_RELATIVE_TO_BOTTOM_OF_CAMERA_VIEW / occupancy_grid.info.resolution
    )
...
def convert_occupancy_grid_index_to_meters(
    occupancy_grid_resolution: float, occupancy_grid_coordinates: OccupancyGridIndex, robot_pose: Pose
) -> Point:
...
def find_closest_drivable_point(occupancy_grid: OccupancyGrid) -> OccupancyGridIndex | None:
    ...
    current_position = OccupancyGridIndex(
        y=0,
        x=-int(ROBOT_FORWARDS_BACKWARDS_POSITION_RELATIVE_TO_BOTTOM_OF_CAMERA_VIEW / occupancy_grid.info.resolution),
    )
    ...
    while len(search_container) > 0:
        ...
        for dy, dx in [(0, 1), (1, 1), (-1, 1), (1, 0), (-1, 0)]:
            potential_position = OccupancyGridIndex(y=index.y + dy, x=index.x + dx)
...
def index_cost(
    occupancy_grid_resolution: float, index: OccupancyGridIndex, robot_pose: Pose, waypoint_meters: Point
) -> float:
    """
    Calculate cost of a node on the occupancy grid as its Euclidean distance from
    the goal node.
    """
    index_meters = convert_occupancy_grid_index_to_meters(occupancy_grid_resolution, index, robot_pose)

    return math.sqrt((index_meters.x - waypoint_meters.x) ** 2 + (index_meters.y - waypoint_meters.y) ** 2)
...
def generate_path_occupancy_grid_indices(
    ...
    null_occ_grid_index = OccupancyGridIndex(
        y=0,
        x=int(ROBOT_FORWARDS_BACKWARDS_POSITION_RELATIVE_TO_BOTTOM_OF_CAMERA_VIEW / occupancy_grid.info.resolution) - 1,
    )
    ...
    while len(priority_queue) > 0:
        ...
        for dy, dx in itertools.product([-1, 0, 1], repeat=2):
            ...
            neighbor = OccupancyGridIndex(y=current_index.y + dy, x=current_index.x + dx)
    ...
    return Path(
        ...
        poses=[
            PoseStamped(
                header=Header(frame_id="odom"),
                pose=Pose(
                    position=convert_occupancy_grid_index_to_meters(occupancy_grid.info.resolution, index, robot_pose)
                ),
            )
