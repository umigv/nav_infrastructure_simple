...
from nav_utils.geometry import distance, point_is_close
...
def smooth_path(points: list[Point], smoothing: float) -> list[Point]:
    ...
    return [Point(x=float(x), y=float(y), z=0.0) for x, y in zip(x_smooth, y_smooth, strict=True)]
